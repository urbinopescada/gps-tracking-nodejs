...
var adapter = function(device) {
  ...
  this.get_ping_data = function(msg_parts) {
    ...
    let dateRaw = str.substr(0, 12);
    let year = "20" + parseInt(dateRaw.slice(0, 2), 16);
    let month = parseInt(dateRaw.slice(2, 4), 16);
    let day = parseInt(dateRaw.slice(4, 6), 16);
    let hour = parseInt(dateRaw.slice(6, 8), 16);
    let minute = parseInt(dateRaw.slice(8, 10), 16);
    let second = parseInt(dateRaw.slice(10, 12), 16);
    let date = new Date(year, month, day, hour, minute, second, "00");
